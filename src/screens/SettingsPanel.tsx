...
export function SettingsPanel({preferences,onPreference,onSave,saving}:Props){
 ...
 return <View>
  <View style={{padding:14,borderRadius:16,borderWidth:1,borderColor:colors.leafTint,backgroundColor:colors.leafMist,flexDirection:'row',gap:10,alignItems:'center',marginBottom:20}}><Ionicons name="cloud-done-outline" size={22} color={colors.leafInk}/><View style={{flex:1}}><Text style={{fontFamily:'NunitoSans_900Black',fontSize:14,color:colors.leafInk}}>Restaurant database settings</Text><Text style={{fontFamily:'NunitoSans_600SemiBold',fontSize:11,color:'#587267',marginTop:3}}>Controls below are loaded from and saved to this workspace.</Text></View></View>
...
function Base({icon,title,children}:{icon:any;title:string;children:React.ReactNode}){return <View style={s.row}><View style={s.icon}><Ionicons name={icon} size={21} color={colors.leafDeep}/></View><Text style={s.rowTitle}>{title}</Text>{children}</View>}
...
function Field({icon,title,value,onChange,keyboard}:any){return <View style={s.field}><View style={s.fieldHead}><Ionicons name={icon} size={20} color={colors.leafDeep}/><Text style={s.rowTitle}>{title}</Text></View><TextInput value={value} onChangeText={onChange} keyboardType={keyboard} style={s.input}/></View>}
const s=StyleSheet.create({section:{marginBottom:24},sectionTitle:{fontFamily:"NunitoSans_900Black",fontSize:19,color:"#111613",marginBottom:10},group:{borderRadius:18,backgroundColor:"#fff",borderWidth:1,borderColor:"#E4E9E6",overflow:"hidden"},row:{minHeight:70,paddingHorizontal:14,flexDirection:"row",alignItems:"center",gap:11,borderBottomWidth:1,borderBottomColor:"#EDF0EE"},icon:{width:38,height:38,borderRadius:12,backgroundColor:colors.leafPale,alignItems:"center",justifyContent:"center"},rowTitle:{flex:1,fontFamily:"NunitoSans_800ExtraBold",fontSize:14,color:"#171C19"},copy:{maxWidth:"43%",fontFamily:"NunitoSans_600SemiBold",fontSize:10.5,lineHeight:14,color:"#727B75",textAlign:"right"},value:{fontFamily:"NunitoSans_800ExtraBold",fontSize:12,color:colors.leafInk,marginRight:3},field:{padding:14,borderBottomWidth:1,borderBottomColor:"#EDF0EE"},fieldHead:{flexDirection:"row",alignItems:"center",gap:10,marginBottom:9},input:{height:43,borderRadius:12,borderWidth:1,borderColor:"#DDE5E0",paddingHorizontal:12,fontFamily:"NunitoSans_700Bold",fontSize:13,color:"#171C19",backgroundColor:"#FCFDFC"},save:{height:52,borderRadius:15,backgroundColor:colors.leaf,alignItems:"center",justifyContent:"center",marginBottom:20},saveText:{fontFamily:"NunitoSans_900Black",fontSize:15,color:"#fff"}});
