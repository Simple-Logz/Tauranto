...
export function HomeScreen({ commands, onCommand, onDecide, onOpenActivity }: { commands: VoiceCommand[]; onCommand: (command: VoiceCommand) => void; onDecide: (id: string, approved: boolean) => void; onOpenActivity: () => void }) {
  ...
  return <ScrollView style={styles.page} contentContainerStyle={[styles.content, { paddingTop: insets.top + 12 }]} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>TAURANTO VOICEOPS</Text><Text style={styles.heading}>Restaurant command center</Text></View><View style={styles.avatar}><Text style={styles.avatarText}>T</Text><View style={styles.online}/></View></View>

    <View style={styles.statusCard}>
      <View style={styles.statusMain}><View style={styles.openIcon}><Ionicons name="shield-checkmark-outline" size={22} color={colors.leafDeep}/></View><View style={{flex:1}}><Text style={styles.statusLabel}>SAFETY STATUS</Text><Text style={styles.statusTitle}>Manager approval is enforced</Text></View></View>
    </View>
...
const styles = StyleSheet.create({
  page:{flex:1,backgroundColor:colors.cream},content:{paddingHorizontal:20,paddingBottom:34},header:{flexDirection:"row",justifyContent:"space-between",alignItems:"center",marginBottom:17},eyebrow:{fontFamily:"DMSans_700Bold",fontSize:12,letterSpacing:1.1,color:colors.leafDeep,marginBottom:6},heading:{fontFamily:"DMSans_700Bold",fontSize:32,lineHeight:37,color:colors.ink,maxWidth:310},avatar:{width:52,height:52,borderRadius:16,backgroundColor:"#F2D6BF",alignItems:"center",justifyContent:"center",borderWidth:2,borderColor:colors.paper,...shadow},avatarText:{fontFamily:"DMSans_700Bold",fontSize:18,color:colors.ink},online:{position:"absolute",right:-2,bottom:-1,width:14,height:14,borderRadius:7,backgroundColor:colors.leaf,borderWidth:2,borderColor:colors.paper},
  statusCard:{flexDirection:"row",alignItems:"center",backgroundColor:colors.paper,borderRadius:18,borderWidth:1,borderColor:colors.line,padding:16,marginBottom:14,...shadow},statusMain:{flex:1,flexDirection:"row",gap:13,alignItems:"center"},openIcon:{width:48,height:48,borderRadius:14,backgroundColor:colors.leafPale,alignItems:"center",justifyContent:"center"},statusLabel:{fontFamily:"DMSans_700Bold",fontSize:11,letterSpacing:.8,color:colors.muted},statusTitle:{fontFamily:"DMSans_700Bold",fontSize:16,color:colors.ink,marginTop:4},statusDivider:{width:1,height:39,backgroundColor:colors.line,marginHorizontal:13},
