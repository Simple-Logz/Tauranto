...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try { const user = await requireUser(req); const restaurantId = String(req.query.restaurantId || ""); const db = admin();
    ...
    const { data, error } = await db.from("commands").select("*,approvals(id,approver_id,status,decided_at),execution_jobs(id,provider,action_key,status,completed_at,verification)").eq("restaurant_id", restaurantId).order("created_at", { ascending: false }).limit(100); if (error) throw error;
