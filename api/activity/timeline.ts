...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 try{
  ...
  const {data:preference}=await db.from('restaurant_preferences').select('privacy').eq('restaurant_id',restaurantId).maybeSingle();
  const retentionDays=Math.max(1,Number(preference?.privacy?.activity_retention_days)||90),cutoff=new Date(Date.now()-retentionDays*86400000).toISOString();
  const [{data:audit,error:auditError},{data:tableEvents,error:tableError}]=await Promise.all([
   db.from('audit_events').select('id,event,metadata,created_at,actor_id,command_id').eq('restaurant_id',restaurantId).gte('created_at',cutoff).order('created_at',{ascending:false}).limit(limit),
   db.from('guest_session_events').select('id,event_type,summary,payload,created_at,actor_id,session_id').eq('restaurant_id',restaurantId).gte('created_at',cutoff).order('created_at',{ascending:false}).limit(Math.min(limit,40))
  ...
  return res.json({activities:activity,retentionDays});
